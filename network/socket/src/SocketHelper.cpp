...
int GetSockAddr(const char* lpszAddress, USHORT usPort, HP_SOCKADDR& addr)
{
	...
	if(::WSAStringToAddressA((char*)lpszAddress, addr.family, nullptr, addr.Addr(), &iSize) != NO_ERROR)
		return FALSE;
...
int IsIPAddress(const char* lpszAddress, EnIPAddrType* penType)
{
	...
	int isOK = GetInAddr(lpszAddress, addr);
...
int GetIPAddress(const char* lpszHost, char* lpszIP, int& iIPLen, EnIPAddrType& enType)
{
	HP_SOCKADDR addr;
...
int EnumHostIPAddresses(const char* lpszHost, EnIPAddrType enType, LPTIPAddr** lpppIPAddr, int& iIPAddrCount)
{
	*lpppIPAddr	 = nullptr;
	...
	int isOK = RetrieveSockAddrIPAddresses(vt, lpppIPAddr, iIPAddrCount);
...
int RetrieveSockAddrIPAddresses(const vector<HP_PSOCKADDR>& vt, LPTIPAddr** lpppIPAddr, int& iIPAddrCount)
{
...
int sockaddr_IN_2_A(const HP_SOCKADDR& addr, ADDRESS_FAMILY& usFamily, char* lpszAddress, int& iAddressLen, unsigned short& usPort)
{
	int isOK	= FALSE;
...
// 		int bIPv6		= addr.IsIPv6();
// 		int bHasPort	= (addr.Port() != 0);
// 
// 		if(!bIPv6)
// 		{
// 			if(bHasPort)
...
int sockaddr_A_2_IN(const char* lpszAddress, unsigned short usPort, HP_SOCKADDR& addr)
{
	addr.family = DetermineAddrFamily(lpszAddress);
...
int SSO_NoBlock(SOCKET sock, int bNoBlock)
{
	return ioctlsocket(sock, FIONBIO, (ULONG*)&bNoBlock);
}

int SSO_NoDelay(SOCKET sock, int bNoDelay)
{
	return setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (CHAR*)&bNoDelay, sizeof(int));
}

int SSO_DontLinger(SOCKET sock, int bDont)
{
	return setsockopt(sock, SOL_SOCKET, SO_DONTLINGER, (CHAR*)&bDont, sizeof(int));
}
...
int SSO_KeepAlive(SOCKET sock, int bKeepAlive)
{
	return setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (CHAR*)&bKeepAlive, sizeof(int));
}
...
int SSO_ReuseAddress(SOCKET sock, int bReuse)
{
	return setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (CHAR*)&bReuse, sizeof(int));
}

int SSO_UDP_ConnReset(SOCKET sock, int bNewBehavior)
{
	int result = NO_ERROR;
	DWORD dwBytes;
...
int ManualCloseSocket(SOCKET sock, int iShutdownFlag, int bGraceful, int bReuseAddress)
{
	if(!bGraceful)
		SSO_Linger(sock, 1, 0);
