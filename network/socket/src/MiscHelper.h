...
template<typename B = void> struct TPackInfo
{
	...
	static TPackInfo* Construct(B* pbuf = nullptr, int head = true, DWORD len = sizeof(DWORD))
	{
	...
	TPackInfo(B* pbuf = nullptr, int head = true, DWORD len = sizeof(DWORD))
...
int AddPackHeader(const WSABUF * pBuffers, int iCount, unique_ptr<WSABUF[]>& buffers, DWORD dwMaxPackSize, USHORT usPackHeaderFlag, DWORD& header);
